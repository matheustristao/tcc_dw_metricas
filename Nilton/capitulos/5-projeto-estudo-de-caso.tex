...
Referente ao contexto deste trabalho, o principal trabalho antecedente foi o desenvolvido por \citeonline{rego_monitoramento_2014}, no qual sua solução será utilizada para monitorar as métricas de código-fonte utilizando \textit{Data Warehouse}.
...
A questão de pesquisa a seguir foi proveniente do problema da falta de capacidade da administração pública de aferir a qualidade interna dos produtos de software adquiridos e desenvolvidos por empresas contratadas, que foi identificado na seção "Problema" do capítulo \ref{chap:introdução}. 
...
Como a busca do entendimento geral sobre o problema de pesquisa foi a partir do estudo de um caso particular, o estudo de caso deste trabalho se caracteriza como instrumental. Podemos observar que a definição de estudo de caso exploratório de \citeonline{yin2001estudo}, onde é elucidado a visão acurada de um caso particular que pode fornecer uma visão geral do problema considerado, é a definição de estudo de caso instrumental na nomenclatura de \cite{stake_art_1995}. 

\section{Seleção}
\label{sec:selecao} 

A instituição selecionada para este estudo de caso foi a CAIXA. O estudo acontecerá mais especificamente na Centralizadora Nacional de Tecnologia da Informação(CETEC) e na Gerência de Filial de suporte Tecnológico de Brasília(GITECBR).
...
Resumindo as atividades realizadas na CETEC no contexto deste trabalho, a CETEC deve analisar a melhor solução entre as  várias disponíveis no mercado para o aferimento da qualidade do produto de \textit{software} adquiridos por empresas terceirizadas. Após a análise, a solução escolhida é implantada e colocada em homologação para a filiais testarem. Se a solução homologada for aprovada pelas filiais a mesma é posta em produção. Atualmente a solução em produção que auxilia as filiais no processo de monitoração de métricas de código-fonte do produto de \textit{software} adquiridos por empresas terceirizadas utiliza a ferramenta SonarQube. Apesar da solução com o sonarQube já está em homologação o processo que definirá quais as métricas, indicadores e os seus valores aceitáveis ainda não foi finalizado. Enquanto este processo não entra em rigor a única métrica que realmente é utilizada para o ateste da qualidade interna do produto de \textit{software} entregue é a métrica número de defeitos e o índice aceitável de defeitos, por ponto de função, é de 0,2 erros e será calculado com base na gravidade de tipos de erros, conforme a Figura \ref{formula}.
...
Para o estudo de caso deste trabalho será analisado um ou dois sistemas do portfólio da filial GITECBR desenvolvido na linguagem java. Em cada \textit{release} de um respectivo projeto a GITECBR recebe um pacote da contratada contendo o código-fonte do sistema, antes da nova versão do sistema entregue ser colocado em homologação, ela deve passar pela solução disponibilizada pela CETEC que utiliza o sonarQube. O sonarQube apresenta os defeitos contidos no código-fonte, com base nos defeitos e no índice aceitável de defeitos o gestor define se a \textit{realease} deve ser rejeitada e se existe alguma multa aplicável conforme a Tabela \ref{tab:multa} e os demais itens.    
...
Os dados deste estudo de caso,serão coletados através de registros de observações em campo, questionários, entrevistas e resultados gerados pela própria solução utilizando o código-fonte de um ou mais sistemas desenvolvidos por empresas contratadas pela CAIXA.

Os registros oriundos de observações em campo são gerados a partir da equipe responsável pela tomada de decisões de qualidade e são coletados durante as visitas realizadas na CAIXA. Nessas visitas, a solução proposta é utilizada e qualquer atitude relacionada ao seu uso pela equipe é registrada. Tais registros são tanto do ponto de vista qualitativo quanto do ponto de vista quantitativo.

A adoção de questionários também foi utilizada tanto para dados qualitativos quanto para dados quantitativos. O questionário fará indagações a respeito da solução de DW pelo ponto de vista da equipe responsável e dos demais envolvidos na qualidade de \textit{software} da CAIXA, no que diz respeito à tomadas de decisões, detecção de cenários de limpeza de código, falhas relacionados à utilização da ferramenta adotada na solução de DW, ao nível de satisfação no uso da solução e as taxas de oportunidades de melhoria de código. 

Os gráficos e resultados gerados pela solução de DW, serão coletados a medida que a solução for utilizada pela equipe de qualidade da CAIXA ao longo das \textit{releases} dos projetos adotados.
